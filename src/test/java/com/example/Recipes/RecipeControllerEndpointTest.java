package com.example.Recipes;//package com.example.Recipes;
...
public class RecipeControllerEndpointTest {
	...
	public void testDeleteRecipeByIdSuccessBehavior() throws Exception {
		...
		// Create a mock Recipe object with id = 3
		Recipe recipe3 = new Recipe();
		recipe3.setId(3L);
		recipe3.setName("Recipe 3");

		// Mock the RecipeService method that retrieves the Recipe object by id
		Mockito.when(recipeService.getRecipeById(3L)).thenReturn(recipe3);

		// Set up the delete request
		mockMvc.perform(delete("/recipes/3"))
		...
		// Verify that the RecipeService deleteRecipeById method was called with the correct id
		verify(recipeService, Mockito.times(1)).deleteRecipeById(3L);
	}
	...
	public void testGetAllRecipesFailureBehavior() throws Exception {
		...
		this.mockMvc.perform(get("/recipes"))
				...
				.andExpect(content().string(containsString("There are no recipes yet :( feel free to add one though"))
				);
